...
#define ADIS16475_HAS_SYNC_4KHZ		BIT(2)
#define ADIS16475_HAS_TIMESTAMP32	BIT(3)
#define ADIS16475_NEEDS_BURST_REQUEST	BIT(4)
#define ADIS16475_HAS_FIFO		BIT(5)
	const long flags;
	u32 num_channels;
	u32 gyro_max_val;
	u32 gyro_max_scale;
...
static int adis16475_set_freq(struct adis16475 *st, const u32 freq)
{
	...
	int ret;
	u32 sample_rate = st->clk_freq;
	u32 max_sample_rate = 2100000;
	u32 min_sample_rate = 1900000;

	if (!freq)
		return -EINVAL;

	if (st->info->flags & ADIS16475_HAS_SYNC_4KHZ) {
		max_sample_rate = 4100000;
		min_sample_rate = 3900000;
	}

	adis_dev_lock(&st->adis);
	/*
	 * When using sync scaled mode, the input clock needs to be scaled so that we have
	 * an IMU sample rate between (optimally) 1900 and 2100 if internal clock frequency
	 * is 2000Hz or between 3900 and 4100 if internal clock frequency is 4000Hz.
	...
	if (st->sync_mode == ADIS16475_SYNC_SCALED) {
		 ...
		 * with a sample rate lower than 2000Hz if internal clock frequency is
		 * 2000Hz or 4000Hz if internal clock frequency is 4000Hz, due to possible
		 ...
		 * we allow sample rates lower than 2KHz/4KHz. By default, we won't allow
		 * this and we just roundup the rate to the next multiple of the input clock
		 * bigger than 2KHz/4KHz. This is done like this as in some cases
...
static int adis16475_buffer_postenable(struct iio_dev *indio_dev)
{
	...
	/* Update watermark, enable watermark interrupt and update overflow behavior. */
	val = ADIS16475_WM_LVL(st->fifo_watermark);
	ret = adis_update_bits(adis, ADIS16475_REG_FIFO_CTRL, ADIS16475_WM_LVL_MASK, val);
	if (ret)
		return ret;
...
static const struct adis16475_chip_info adis16475_chip_info[] = {
	[ADIS16470] = {
		...
		.adis_data = ADIS16475_DATA(16470, &adis16475_timeouts, ADIS16475_BURST32_MAX_DATA, ADIS16475_BURST_MAX_SPEED),
	},
	[ADIS16475_1] = {
		.name = "adis16475-1",
		.num_channels = ARRAY_SIZE(adis16475_channels),
		...
		.sync = adis16475_sync_mode,
		.num_sync = ARRAY_SIZE(adis16475_sync_mode),
		.adis_data = ADIS16475_DATA(16475, &adis16475_timeouts, ADIS16475_BURST32_MAX_DATA, ADIS16475_BURST_MAX_SPEED),
	},
	[ADIS16475_2] = {
		.name = "adis16475-2",
		.num_channels = ARRAY_SIZE(adis16475_channels),
	...
	[ADIS16477_1] = {
		...
		.adis_data = ADIS16475_DATA(16477, &adis16475_timeouts, ADIS16475_BURST32_MAX_DATA, ADIS16475_BURST_MAX_SPEED),
	},
	[ADIS16477_2] = {
		.name = "adis16477-2",
		.num_channels = ARRAY_SIZE(adis16477_channels),
	...
	[ADIS16465_1] = {
		...
		.sync = adis16475_sync_mode,
		.num_sync = ARRAY_SIZE(adis16475_sync_mode),
		.adis_data = ADIS16475_DATA(16465, &adis16475_timeouts, ADIS16475_BURST32_MAX_DATA, ADIS16475_BURST_MAX_SPEED),
	},
	[ADIS16465_2] = {
		.name = "adis16465-2",
		.num_channels = ARRAY_SIZE(adis16475_channels),
	...
	[ADIS16467_1] = {
		...
		.sync = adis16475_sync_mode,
		.num_sync = ARRAY_SIZE(adis16475_sync_mode),
		.adis_data = ADIS16475_DATA(16467, &adis16475_timeouts, ADIS16475_BURST32_MAX_DATA, ADIS16475_BURST_MAX_SPEED),
	},
	[ADIS16467_2] = {
		.name = "adis16467-2",
		.num_channels = ARRAY_SIZE(adis16475_channels),
	...
	[ADIS16500] = {
		...
		.flags = ADIS16475_HAS_BURST32 | ADIS16475_HAS_BURST_DELTA_DATA,
		.adis_data = ADIS16475_DATA(16500, &adis1650x_timeouts, ADIS16475_BURST32_MAX_DATA, ADIS16475_BURST_MAX_SPEED),
	},
	[ADIS16501] = {
		.name = "adis16501",
		.num_channels = ARRAY_SIZE(adis16477_channels),
		...
		.num_sync = ARRAY_SIZE(adis16475_sync_mode) - 1,
		.flags = ADIS16475_HAS_BURST32 | ADIS16475_HAS_BURST_DELTA_DATA,
		.adis_data = ADIS16475_DATA(16501, &adis1650x_timeouts, ADIS16475_BURST32_MAX_DATA, ADIS16475_BURST_MAX_SPEED),
	},
	[ADIS16505_1] = {
		.name = "adis16505-1",
		.num_channels = ARRAY_SIZE(adis16477_channels),
		...
		.num_sync = ARRAY_SIZE(adis16475_sync_mode) - 1,
		.flags = ADIS16475_HAS_BURST32 | ADIS16475_HAS_BURST_DELTA_DATA,
		.adis_data = ADIS16475_DATA(16505, &adis1650x_timeouts, ADIS16475_BURST32_MAX_DATA, ADIS16475_BURST_MAX_SPEED),
	},
	[ADIS16505_2] = {
		.name = "adis16505-2",
		.num_channels = ARRAY_SIZE(adis16477_channels),
	...
	[ADIS16507_1] = {
		...
		.num_sync = ARRAY_SIZE(adis16475_sync_mode) - 1,
		.flags = ADIS16475_HAS_BURST32 | ADIS16475_HAS_BURST_DELTA_DATA,
		.adis_data = ADIS16475_DATA(16507, &adis1650x_timeouts, ADIS16475_BURST32_MAX_DATA, ADIS16475_BURST_MAX_SPEED),
	},
	[ADIS16507_2] = {
		.name = "adis16507-2",
		.num_channels = ARRAY_SIZE(adis16477_channels),
	...
	[ADIS16575_2] = {
		...
		.flags = ADIS16475_HAS_BURST32 | ADIS16475_HAS_BURST_DELTA_DATA | ADIS16475_NEEDS_BURST_REQUEST | ADIS16475_HAS_TIMESTAMP32 | ADIS16475_HAS_SYNC_4KHZ | ADIS16475_HAS_FIFO,
		.adis_data = ADIS16475_DATA(16575, &adis1650x_timeouts, ADIS16575_BURST32_MAX_DATA, ADIS16575_BURST_MAX_SPEED),
	},
	[ADIS16575_3] = {
		.name = "adis16575-3",
		.num_channels = ARRAY_SIZE(adis16477_channels),
	...
	[ADIS16576_2] = {
		...
		.num_sync = ARRAY_SIZE(adis16575_sync_mode),
		.flags = ADIS16475_HAS_BURST32 | ADIS16475_HAS_BURST_DELTA_DATA | ADIS16475_NEEDS_BURST_REQUEST | ADIS16475_HAS_TIMESTAMP32 | ADIS16475_HAS_SYNC_4KHZ | ADIS16475_HAS_FIFO,
		.adis_data = ADIS16475_DATA(16576, &adis1650x_timeouts, ADIS16575_BURST32_MAX_DATA, ADIS16575_BURST_MAX_SPEED),
	},
	[ADIS16576_3] = {
		.name = "adis16576-3",
		.num_channels = ARRAY_SIZE(adis16477_channels),
	...
	[ADIS16577_2] = {
		...
		.num_sync = ARRAY_SIZE(adis16575_sync_mode),
		.flags = ADIS16475_HAS_BURST32 | ADIS16475_HAS_BURST_DELTA_DATA | ADIS16475_NEEDS_BURST_REQUEST | ADIS16475_HAS_TIMESTAMP32 | ADIS16475_HAS_SYNC_4KHZ | ADIS16475_HAS_FIFO,
		.adis_data = ADIS16475_DATA(16577, &adis1650x_timeouts, ADIS16575_BURST32_MAX_DATA, ADIS16575_BURST_MAX_SPEED),
	},
	[ADIS16577_3] = {
		.name = "adis16577-3",
		.num_channels = ARRAY_SIZE(adis16477_channels),
...
static int adis16475_push_single_sample(struct iio_poll_func *pf)
{
	...
	if (st->info->flags & ADIS16475_NEEDS_BURST_REQUEST) {
		for (idx = 0; idx < burst_size; idx++)
...
static irqreturn_t adis16475_trigger_handler_with_fifo(int irq, void *p)
{
	...
	st->burst_request[0] = 0;
	st->burst_request[1] = 0;
	adis16475_push_single_sample(pf);
	st->burst_request[0] = 0x68;
	st->burst_request[1] = 0;
...
static int adis16475_probe(struct spi_device *spi)
{
	...
	if (st->info->flags & ADIS16475_HAS_FIFO) {
		st->burst_request[0] = 0x68;
		st->burst_request[1] = 0;
		st->adis.adis_burst_request = st->burst_request;
		ret = devm_adis_setup_buffer_and_trigger_with_fifo(&st->adis, indio_dev,
