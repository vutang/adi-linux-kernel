		...
		struct jesd204_link *lnk)
{
	...
	dev_info(dev, "JESD Link: ID %d, sample_rate %ld, \
		sample_rate_div %d, num_lanes %d, num_converters %d, \
		bits_per_sample %d, jesd_encoder %d\n", lnk->link_id, 
		lnk->sample_rate, lnk->sample_rate_div, lnk->num_lanes, lnk->num_converters,\
...
static int axi_jesd204_tx_probe(struct platform_device *pdev)
{
	...
	clk_rate = clk_get_rate(jesd->device_clk);
	printk("%s: clk_get_rate device_clk to %d\n", __func__, clk_rate);
	...
	printk("%s: clk_get_rate lane_clk to %d\n", __func__, clk_rate);
	...
	dev_info(&pdev->dev, "%s AXI-JESD204-TX (%d.%.2d.%c) at 0x%08llX. Encoder %s, width %u/%u, lanes %d%s.",
		__func__,
