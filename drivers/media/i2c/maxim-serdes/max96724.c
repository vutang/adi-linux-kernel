					    ...
					    unsigned long config)
{
	...
	if (param == MAX96724_PINCTRL_JITTER_COMPENSATION_EN) {
		max96724_update_bits(priv, 0x337 + offset * 0x3, BIT(6), 0b1);
		max96724_update_bits(priv, 0x36d + offset * 0x3, BIT(6), 0b1);
		max96724_update_bits(priv, 0x3a4 + offset * 0x3, BIT(6), 0b1);
	}
